...
{	
	...
	public function reporteAdeudos(){
		...
		$this->db->where("tbl_cliente.RFC = tbl_traslados.IDCLIENTE  AND tbl_traslados.IDVEHICULO = tbl_vehiculos.IDVEHICULO AND  tbl_vehiculos.IDMODELO = tbl_modelo.IDMODELO AND tbl_traslados.FECHA BETWEEN '$this->fecha_ini' AND '$this->fecha_fin' AND tbl_traslados.PAGADO = 'NO'");
	...
	public function adeudos(){
		...
		$this->fecha_fin = $this->input->post('txt_fecha_fin');
		$this->id_cliente = $this->input->post('adeudo');
		$this->db->select("cli.R_SOCIAL,cli.NOMBRE,cli.APEPAT,cli.APEMAT,tras.FECHA,CONCAT('$', FORMAT(tras.MONTO, 2)) as MONTO,tras.HORA,CONCAT(tras.LUGAR_REF, ' - ' , tras.DOMICILIO ) as RUTA, tras.NOMBRE_PASAJERO,tras.NOMBRE_SOLICITANTE,tras.BAUCHER,tras.CECO,tras.IDCOMPROBANTE",FALSE);
